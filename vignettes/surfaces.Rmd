...
```{r sfc_blues}
sfc_blues <- pth_new_surface(c("#42B4E6", "#0087CD"), transformer = pth_to_jzazbz100)
pth_plot_surface(sfc_blues)
``` 
```{r sfc_oranges}
sfc_oranges <- pth_new_surface(c("#E47F00", "#702407"), transformer = pth_to_jzazbz100)
pth_plot_surface(sfc_oranges)
```    

```{r sfc_greens}
sfc_greens <- pth_new_surface(c("#007626", "#70E07C"), transformer = pth_to_jzazbz100)
pth_plot_surface(sfc_greens)
```


```{r sfc_purples}
sfc_purples <- pth_new_surface(c("#3D1152", "#C530FF"), transformer = pth_to_jzazbz100)
pth_plot_surface(sfc_purples)
```
