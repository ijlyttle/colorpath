...
Luminance propels everything. Chroma used to create a longer path. Be careful with hue because differences in hue are perceived differently by folks with color-vision deficiency.  
...
```{r}
sfc_multi_blue <- surface_hl(c(273, 215))
...
```{r}
pal_blues_light <- pal_luv_bezier(as_mat_luv(dfhcl_blues_light), rescale = FALSE)
...
The two plots seem inconsistent. Even though the 
